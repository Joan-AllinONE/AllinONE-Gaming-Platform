...
class WalletService {
  ...
  private readonly DEFAULT_EXCHANGE_RATES: ExchangeRate = {
    gameCoinsToRMB: 0.01,        // 1 AllinONE 游戏币 = 0.01元
    newDayGameCoinsToRMB: 0.01,  // 1 New Day 游戏币 = 0.01元
    computingPowerToRMB: 0.001,  // 1算力 = 0.001元
    gameCoinsToNewDay: 1,        // 1 AllinONE 游戏币 = 1 New Day 游戏币
    newDayToGameCoins: 1,        // 1 New Day 游戏币 = 1 AllinONE 游戏币
  ...
  private async initializeWalletData(): Promise<void> {
    ...
    if (!existingData) {
      const initialWallet = {
        balance: {
          cash: 0,
          gameCoins: 12580,       // AllinONE 游戏币
          newDayGameCoins: 0,     // New Day 游戏币（初始为0，从New Day同步）
  ...
  private async getWalletData() {
    ...
    if (walletData.balance) {
      ...
      // 🔥 数据迁移：添加 newDayGameCoins 字段
      if (typeof walletData.balance.newDayGameCoins === 'undefined') {
        walletData.balance.newDayGameCoins = 0; // 为现有用户添加初始New Day游戏币余额
        needsUpdate = true;
      }
  ...
  private async calculateTotalValue(balance: WalletBalance): Promise<number> {
    ...
    return balance.cash + 
           ...
           (balance.newDayGameCoins * (rates.newDayGameCoinsToRMB || 0.01)) + // New Day 游戏币价值
  ...
  async addTransaction(transaction: Omit<WalletTransaction, 'id' | 'timestamp'>): Promise<void> {
    ...
    console.log('💳 WalletService.addTransaction - 当前余额:', {
    ...
    console.log('💳 WalletService.addTransaction - 更新后余额:', {
      currency: transaction.currency,
