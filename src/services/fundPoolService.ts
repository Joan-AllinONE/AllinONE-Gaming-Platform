...
class FundPoolService {
  ...
  private exchangeRates = {
    ...
    gameCoins: 0.01,        // 1 AllinONE 游戏币 = 0.01现金
    newDayGameCoins: 0.01,  // 1 New Day 游戏币 = 0.01现金
    aCoins: 1,              // 1A币 = 1现金
    cash: 1,                // 1现金 = 1现金
  ...
  private updateBalance(): void {
    ...
    newBalance.totalValue = 
      ...
      newBalance.newDayGameCoins * this.exchangeRates.newDayGameCoins +
    ...
    newBalance.newDayGameCoins = Math.round(newBalance.newDayGameCoins * 100) / 100;
    // A币余额始终为0，因为发放即转移给用户
    newBalance.aCoins = 0;
    // O币余额始终为0，因为发放即转移给用户
    newBalance.oCoins = 0;
  ...
  async getStats(): Promise<FundPoolStats> {
    ...
    // 计算总收入和支出
    const totalIncome = { computingPower: 0, cash: 0, gameCoins: 0, newDayGameCoins: 0, aCoins: 0, oCoins: 0, totalValue: 0 };
    const totalExpense = { computingPower: 0, cash: 0, gameCoins: 0, newDayGameCoins: 0, aCoins: 0, oCoins: 0, totalValue: 0 };
    ...
    const netIncome = {
      ...
      newDayGameCoins: totalIncome.newDayGameCoins - totalExpense.newDayGameCoins,
