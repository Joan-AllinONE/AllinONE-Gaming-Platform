...
import NewDayStore from '@/components/NewDayStore';
import { NewDayItemType } from '@/types/newDay';
...
export default function OfficialStore() {
  ...
  const [showNewDayStore, setShowNewDayStore] = useState(false);
  const [selectedNewDayCategory, setSelectedNewDayCategory] = useState<NewDayItemType | undefined>(undefined);
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden md:flex items-center gap-4 px-4 py-2 bg-gradient-to-r from-green-50 to-blue-50 dark:from-green-900/20 dark:to-blue-900/20 rounded-lg border border-green-200 dark:border-green-800">
                ...
                <div className="relative flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                    {(userWallet?.gameCoins + (userWallet?.newDayGameCoins || 0)).toLocaleString() ?? '0'}
                  </span>
                  <button
                    ...
                    <div className="absolute top-full right-0 mt-2 w-64 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 z-50 p-3">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
                          ...
                          <span className="text-sm font-bold text-slate-900 dark:text-slate-100">{(userWallet?.gameCoins ?? 0).toLocaleString()}</span>
                        </div>
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {/* AllinONE 官方商店和 New Day 商店切换 */}
            <button
              onClick={() => setShowNewDayStore(false)}
            ...
            <button
              onClick={() => setShowNewDayStore(true)}
              className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${
                showNewDayStore
                  ? 'bg-purple-600 text-white'
                  : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-purple-50 dark:hover:bg-slate-700'
        ...
        {/* 商品网格 */}
        {showNewDayStore ? (
          <NewDayStore selectedCategory={selectedNewDayCategory} />
        ) : (
